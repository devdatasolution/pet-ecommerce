    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" enctype="multipart/form-data" class="d-block ajaxForm"
                action="{{ route('install.validate') }}">
                @csrf
                <div class="form-group">
                    <label for="purchase_code" class="control-label mb-3">{{ __('Purchase Code') }}</label>
                    <input type="text" class="form-control eForm-control" name="purchase_code"
                        placeholder="Product's Purchase Code" required autofocus autocomplete="off">
                </div>
            ...
            <div class="text-center">
                <p>{{ __('Need any help?') }} <a href="https://creativeitem.com/">{{ __('Contact Us') }}</a></p>
            </div>
        </div>
    </div>
@endsection
