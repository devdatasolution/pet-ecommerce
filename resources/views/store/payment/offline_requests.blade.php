...
@push('title', get_phrase('Offline Payment requests'))
@push('meta')
@endpush
@push('css')
@endpush
                        ...
                        <tbody>
                            @foreach ($payments as $key => $payment)
                                <tr>
                                    ...
                                    <td>
                                        #{{ implode(', ', $payment->store_orders->pluck('id')->map(fn($id) => $id + 100)->toArray()) }}
                                    </td>
                                    <td>
                                        <span class="fw-600">{{ $payment->currency_code }} {{ $payment->store_total_amount }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        @endphp
                                        <strong>{{ get_phrase('Bank No.') }}:</strong> {{ $payment_details->bank_no ?? 'N/A' }}<br>
                                        <strong>{{ get_phrase('Phone No.') }}:</strong> {{ $payment_details->phone_no ?? 'N/A' }}<br>
                                        <strong>{{ get_phrase('Document') }}:</strong> 
                                        @if (isset($payment_details->file_path))
                                            <a href="#" onclick="ajaxModal('{{ route('view', ['path' => 'store.payment.view_doc', 'doc' => $payment_details->file_path]) }}', '{{ get_phrase('Proof of payment') }}', 'modal-lg')" data-bs-toggle="tooltip" title="{{ get_phrase('View proof') }}" class="btn btn-light btn-icon m-1">
                                    ...
                                    <td>
                                        <div class="print-d-none d-flex flex-wrap align-items-center gap-2 row-gap-2">
                                            @if($payment->status == 'pending' || $payment->status == 'unpaid')
                                                <a href="javascript:;" onclick="confirmModal('{{ route('vendor.payment.offline_status', ['id' => $payment->id, 'status' => 'paid']) }}')" data-bs-toggle="tooltip" title="{{ get_phrase('Approve') }}" class="btn btn-success-light btn-icon m-1"><i class="fi-rr-check"></i></a>
                                            @endif
                                            @if($payment->status == 'pending')
                                                <a href="javascript:;" onclick="confirmModal('{{ route('vendor.payment.offline_status', ['id' => $payment->id, 'status' => 'unpaid']) }}')" data-bs-toggle="tooltip" title="{{ get_phrase('Reject') }}" class="btn btn-danger-light btn-icon m-1"><i class="fi-rr-cross"></i></a>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
