                ...
                <div class="message-sidebar-area">
                    <div class="message-sidebar-header">
                        <div class="back-and-plus mb-3 d-flex align-items-center justify-content-between flex-wrap">
                            ...
                            <a href="#" onclick="ajaxModal('{{ route('view', ['path' => 'store.message.message_new']) }}', '{{ get_phrase('Create a new thread') }}')" data-bs-toggle="tooltip" title="{{ get_phrase('New message') }}" class="btn ol-btn-light ol-icon-btn ol-icon-btn-sm">
                        ...
                        <form action="#">
                            <div class="message-sidebar-search">
                                ...
                                <input type="search" class="form-control" onkeyup="load_view('{{ route('view', ['path' => 'store.message.message_left_sidebar']) }}?thread_id={{ $thread_details->id }}&search='+$(this).val(), '#message-user-list')" id="message-sideSearch" placeholder="{{ get_phrase('Search user') }}">
                    ...
                    <ul class="message-sidebar-messages" id="message-user-list">
                        @include('store.message.message_left_sidebar')
                    </ul>
                </div>
            </div>
            <div class="col-xl-7 col-lg-6 col-md-7">
                @if ($thread_details->id > 0)
                    @include('store.message.message_body')
                @else
