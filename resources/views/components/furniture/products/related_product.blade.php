...
<!-- Swiper -->
<div class="swiper products-sliders">
    <div class="swiper-wrapper">
               {{-- @foreach ($product->related_products()->where('status', 1)->take(30) as $related_product) --}}
                ...
                <div class="product-md-card swiper-slide">
                    <div class="product-md-banner mb-3">
                        ...
                        <div class="product-md-buttons">
                            <a href="javascript:;"  class="white-icon-btn {{ wishlist_class($related_product->id) }}" 
                            ...
                            <a href="javascript:void(0)" class="white-icon-btn" onclick="load_view('{{ route('view', ['path' => 'frontend.products.quick_view', 'product_id' => $related_product->id]) }}', '#quickViewModal .modal-body')"  data-bs-toggle="modal" data-bs-target="#quickViewModal">
                    ...
                    <div class="product-md-details">
                        <div class="d-flex align-items-center gap-1 mb-2">
                            ...
                            <span class="al-subtitle2-14px">({{ $related_product->reviews->count() }}) {{get_phrase('Reviews')}}</span>
                        </div>
                        <a href="{{ route('product', $related_product->slug) }}" class="al-title-16px text-link mb-20px">{{ \Illuminate\Support\Str::limit($related_product->title, 70, '...') }}</a>
                               ...
                               @if ($related_product->is_discounted()->exists())
                                    @if ($related_product->is_discounted->discount_type == 'flat')
                                        <div class="d-flex gap-2">
                                            <h6 class="al-title2-18px">  {{ currency($related_product->price - $related_product->is_discounted->discount_value) }} </h6>
                                            <h6 class="al-title-16px fw-medium fsh-text-gray"><del>{{ currency($related_product->price) }}</del></h6>
                                        </div>
                                    @else
                                        @php
                                        ...
                                        @endphp
                                        <div class="d-flex gap-2">
                                            <h6 class="al-title2-18px"> {{ currency($related_product->price - $discount_amount) }}  </h6>
                                                <h6 class="al-title-16px fw-medium fsh-text-gray"><del>{{ currency($related_product->price) }}</del></h6>
                                        </div>
                                        
                                    @endif
                                @else
                                        <h6 class="al-title2-18px">{{ currency($related_product->price) }}</h6>
                                @endif
                            </div>
                        </div>
                    </div>
