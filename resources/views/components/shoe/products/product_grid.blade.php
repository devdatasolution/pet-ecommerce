...
<div class="sh1-product-{{ $layout }}-card-outer">
      @if ($product->is_discounted()->exists())
            ...
            <p class="product-{{ $layout }}-card-badge">{{ $discount_text }}</p>
        @endif
    <div class="sh1-product-{{ $layout }}-card">
        <div class="d-flex align-items-start gap-2 justify-content-between mb-18px">
            <a href="{{ route('product', $product->slug) }}" class="product-{{ $layout }}-card-title">{{ $product->title }}</a>
            <a href="javascript:;" class="product-{{ $layout }}-card-wishlist {{ wishlist_class($product->id) }}" 
        ...
        <!-- Swiper -->
        <div class="swiper product-{{ $layout }}-card-slider">
            <div class="swiper-wrapper">
                <div class="swiper-slide">
                    <div class="product-{{ $layout }}-card-slide">
                        @php
                ...
                @if ($product->is_discounted()->exists())
                    @if ($product->is_discounted->discount_type == 'flat')
                        <div class="d-flex align-items-baseline gap-2">
                            <h6 class="product-{{ $layout }}-card-price">  {{ currency($product->price - $product->is_discounted->discount_value) }} </h6>
                    ...
                    @else
                        ...
                        @endphp
                        <div class="d-flex align-items-baseline gap-2">
                            <h6 class="product-{{ $layout }}-card-price"> {{ currency($product->price - $discount_amount) }}  </h6>
                                <h6 class="al-title-14px fw-medium fsh-text-gray"><del>{{ currency($product->price) }}</del></h6>
                        </div>
                        
                    @endif
                @else
                        <h6 class="product-{{ $layout }}-card-price">{{ currency($product->price) }}</h6>
                @endif
                <div class="d-flex align-items-center gap-1">
                    <span class="product-{{ $layout }}-card-star">
                    ...
                    <h4 class="product-{{ $layout }}-card-rating">{{ number_format($product->average_rating, 1) }}</h4>
                </div>
            </div>
            <a href="{{ route('product', $product->slug) }}" class="btn sh1-sm-btn-dark btn-{{ $layout }} text-nowrap">
                    {{ get_phrase('Shop Now') }}
            </a>
        </div>
    </div>
