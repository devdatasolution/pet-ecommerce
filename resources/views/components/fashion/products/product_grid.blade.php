...
<div class="d-block product-grid-{{ $layout }}">
    <div>
        ...
        @endif
            ...
            <a href="{{ route('product', $product->slug) }}"  class="btn fsh-btn-dark product-cart-btn-{{ $layout }} grid4-product-cart-btn">
                    {{ get_phrase('Shop Now') }}
            </a>
            <a href="javascript:void(0)"  class="product-wishlist-btn grid4-wishlist {{ wishlist_class($product->id) }}"   onclick="toggleWishlist({{ $product->id }}, this)">
            ...
            <a href="javascript:void(0);" onclick="load_view('{{ route('view', ['path' => 'frontend.products.quick_view', 'product_id' => $product->id]) }}', '#quickViewModal .modal-body')" class="product-quickview-btn grid4-quickview" data-bs-toggle="modal" data-bs-target="#quickViewModal">
