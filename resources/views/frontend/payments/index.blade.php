                ...
                <div class="col-xl-9 col-lg-8">
                    ...
                    <div class="d-flex align-items-start justify-content-between gap-2 mb-20px">
                        <div class="d-flex justify-content-between align-items-start align-items-lg-center gap-12px flex-column flex-lg-row w-100">
                            <h1 class="in-title-16px text-white-color">{{ get_phrase('Payment history') }}</h1>
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb up-breadcrumb text-white-color">
                                  ...
                                  <li class="breadcrumb-item up-breadcrumb-item active" aria-current="page">{{ get_phrase('Payment history') }}</li>
                                </ol>
                            </nav>
                        </div>
                            ...
                            <table class="table up-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" class="in-title-14px text-nowrap">{{ get_phrase('Order ID') }}</th>
                                        <th scope="col" class="in-title-14px text-nowrap">{{ get_phrase('Total amount') }}</th>
                                        <th scope="col" class="in-title-14px text-nowrap">{{ get_phrase('Payment status') }}</th>
                                        <th scope="col" class="in-title-14px text-nowrap">{{ get_phrase('Payment method') }}</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <p class="in-subtitle-14px lh-1 text-nowrap up-text-dark">
                                                {{ $payment->currency_code }} {{ $payment->total_amount }}
                                            </p>
                                        </td>
                                        <td>
                                            @if ($payment->status == 'paid')
                                                <p class="up-badge-success">{{ ucfirst($payment->status) }}</p>
                                            @elseif($payment->status == 'pending')
                                                <p class="up-badge-danger">{{ ucfirst($payment->status) }}</p>
                                            @else
                                                <p class="up-badge-yellow">{{ ucfirst($payment->status) }}</p>
                                            @endif
                                        </td>
                                        <td>
                                            @if($payment->payment_method == 'offline')
                                               ...
                                               {{ get_phrase('Document') }}:
                                                ...
                                                @endif
                                            @endif
                                            <p class="up-badge-dark">{{ ucfirst($payment->payment_method) }}</p>
                                        </td>
                                        <td>
                                            <a href="{{ route('customer.payment.invoice', ['payment_id' => $payment->id]) }}" class="btn up-btn-light text-nowrap">
