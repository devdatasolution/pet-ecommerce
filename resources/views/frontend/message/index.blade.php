            ...
            <div class="col-xl-9 col-lg-8">
                ...
                <div class="d-flex align-items-start justify-content-between gap-2 mb-20px">
                    <div class="d-flex justify-content-between align-items-start align-items-lg-center gap-12px flex-column flex-lg-row w-100">
                        <h1 class="in-title-16px text-white-color">{{ get_phrase('Messages') }}</h1>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb up-breadcrumb">
                                <li class="breadcrumb-item up-breadcrumb-item"><a href="{{ route('home') }}">{{ get_phrase('Home') }}</a></li>
                                <li class="breadcrumb-item up-breadcrumb-item active" aria-current="page">{{ get_phrase('Messages') }}</li>
                            </ol>
                        </nav>
                    </div>
                    <button class="btn ca-menu-btn-primary d-lg-none" type="button" data-bs-toggle="offcanvas"
                        data-bs-target="#user-sidebar-offcanvas" aria-controls="user-sidebar-offcanvas">
                    ...
                    <div class="row">
                        <div class="col-md-4 horigontal-border">
                            ...
                            <ul class="mt-4 messageControll pb-4 mb-4">
                                ...
                                <li class="mb-2">
                                    <a href="{{ route('customer.messages', ['id' => $thread->user_one == auth()->user()->id ? $thread->user_two : $thread->user_one, 'thread_id' => $thread->id]) }}"
                                        class="d-flex align-items-center message-thread thread-active">
                                        <div class="circle-img-50px">
                                            <img src="{{ get_image($thread->user->photo) }}" width="50px" height="50px" alt="">
                                        </div>
                                        <div class="ps-2">
                        ...
                        <div class="col-md-8">
                            ...
                            <h4 class="in-title-16px mt-3 mt-sm-0">
                                ...
                                {{ $thread_details->user->name }}
                                @endif
                            </h4>
                            <div class="mt-4">
                                @if(isset($messages) || isset($thread_details))
                                    @if (count($messages) > 0)    
                                                ...
                                                @else
                                                    <li class="w-100 my-2">
                                                        <div class="d-flex align-items-center justify-content-end circle-img-50pxs w-100 my-3 float-end clearfix">
                                                            ...
                                                            <img src="{{ get_image(auth()->user()->photo) }}" width="50px" height="50px" alt="">
                                                        </div>
                                                    </li>
                                                @endif
                                            @endforeach
                                    ...
                                    @endif
                                    <form action="{{ route('customer.message.send',['thread_id'=>$thread->id]) }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        <input type="hidden" name="receiver_id" value="{{ $thread_details->receiver_id }}" >
