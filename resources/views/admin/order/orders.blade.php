    ...
    <div class="ol-card">
        <div class="ol-card-body p-3 mb-5">
            <div class="row mb-4 mt-1">
                <div class="col-lg-9 mb-1 col-sm d-flex align-items-center flex-wrap gap-3">
                    <style>
                        .filterOption .select2-selection {
                            width: 140px;
                        }
                    </style>
                     <form id="filter-dropdown" action="{{ route('admin.orders') }}" method="get" >
                    ...
                    @if (isset($_GET) && count($_GET) > 0)
                        <a href="{{ route('admin.orders') }}" class="me-2" data-bs-toggle="tooltip" title="{{ get_phrase('Clear') }}"><i class="fi-rr-cross-circle"></i></a>
                    @endif
                ...
                <div class="col-lg-3 mb-1 mt-md-0 mt-3">
                        ...
                        <tbody>
                            @foreach ($orders as $key => $order)
                                <tr>
                                        ...
                                        <div class="d-flex align-items-center min-w-200px">
                                            ...
                                            <div class="ms-2 mt-1">
                                                ...
                                                @if (!empty($addressParts))
                                                    <a class="btn btn-light btn-icon m-1" data-bs-toggle="tooltip" title="{{ get_phrase('Location') }}" href="https://www.google.com/maps/search/?api=1&query={{ urlencode($fullAddress) }}" target="_blank" title="{{ get_phrase('View on Map') }}">
                                    ...
                                    <td>
                                        {{-- @php
                                            $status = $order->order_updates()->latest()->first()->order_status;
                                        @endphp 
                                        <span class="badge text-uppercase" style="background-color: {{ $status->color }}30; color: {{ $status->color }};">{{ get_phrase($status->name) }}</span> --}}
                                         @php
                                    ...
                                    <td width="200px">
                                        ...
                                        <span class="badge bg-dark text-uppercase mt-1">{{ $order->payment_method }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td class="print-d-none">
                                        <a href="{{ route('admin.order.details', ['id' => $order->id]) }}" data-bs-toggle="tooltip" data-bs-title="{{ get_phrase('View details') }}" class="btn btn-success-light btn-icon m-1"><i class="fi-rr-money-check"></i></a>
                                        
                                        <a href="javascript:;" onclick="confirmModal('{{ route('admin.order.delete', ['id' => $order->id]) }}')" data-bs-toggle="tooltip" data-bs-title="{{ get_phrase('Delete') }}" class="btn btn-danger-light btn-icon m-1"><i class="fi-rr-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
