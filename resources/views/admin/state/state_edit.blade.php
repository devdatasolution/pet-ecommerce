...
@push('title', get_phrase('State Edit'))
@push('meta')
@endpush
@push('css')
@endpush
        ...
        <div class="col-md-7">
            <div class="ol-card p-4">
                <div class="ol-card-body">
                    <form action="{{ route('admin.state.update', ['id' => $state->id]) }}" method="post" enctype="multipart/form-data">
                        @csrf

                        <div class="mb-3">
                            <label for="name" class="form-label ol-form-label">{{ get_phrase('State name') }}</label>
                            <input type="text" value="{{ $state->name }}" name="name" class="form-control ol-form-control" id="name" placeholder="{{ get_phrase('Enter state name') }}" aria-label="{{ get_phrase('Enter state name') }}" required>
                        </div>
