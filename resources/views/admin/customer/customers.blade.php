...
@push('title', get_phrase('Customer List'))
@push('meta')
@endpush
@push('css')
@endpush
        ...
        <div class="ol-card-body p-3 mb-5">
            <div class="row mb-4">
                <div class="col-md-6 d-flex flex-wrap align-items-center gap-3">
                    <a href="{{ route('admin.customer.add') }}" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px">
                        <span class="fi-rr-plus"></span>
                        <span>{{ get_phrase('Add new Customer') }}</span>
                    </a>
                    <div class="custom-dropdown ms-2">
                        ...
                        <ul class="dropdown-list">
                            <li>
                                <a class="dropdown-item export-btn" href="javascript:;" onclick="downloadPDF('.print-table', 'Customer-list')"><i class="fi-rr-file-pdf"></i> {{ get_phrase('PDF') }}</a>
                            </li>
                            <li>
                    ...
                    <div class="custom-dropdown dropdown-filter @if (!isset($_GET) || (isset($_GET) && count($_GET) == 0))  @endif">
                        ...
                        <ul class="dropdown-list w-250px">
                            <li>
                                <form id="filter-dropdown" action="{{ route('admin.customers') }}" method="get">
                    ...
                    @if (isset($_GET) && count($_GET) > 0)
                        <a href="{{ route('admin.customers') }}" class="me-2" data-bs-toggle="tooltip" title="{{ get_phrase('Clear') }}"><i class="fi-rr-cross-circle"></i></a>
                    @endif
                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-7 col-10">
                                <div class="search-input flex-grow-1">
                                    <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ get_phrase('Search by customer name or email') }}" class="ol-form-control form-control" />
                                </div>
                            </div>
                            <div class="col-2">
            ...
            @if ($counted = $customers->count() > 0)
                    ...
                    <table class="table print-table">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">{{ get_phrase('Gender') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center min-w-200px">
                                            <img class="image-40" width="40" height="40" src="{{ get_image($customer->photo) }}">
                                            <div class="ms-2 mt-1">
                                                <h4 class="title fs-14px">{{ $customer->name }}</h4>
                                                <p class="sub-title2 text-12px">{{ $customer->email }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <a class="btn btn-light btn-icon" href="tel:{{ $customer->phone }}"><i class="fi-rr-phone-plus"></i></a> {{ $customer->phone }}
                                    </td>
                                    <td>
                                        {{ $customer->address }}
                                    </td>
                                    ...
                                    <td>
                                        <div class="print-d-none d-flex flex-wrap align-items-center gap-2 row-gap-2">
                                            <a href="{{ route('admin.customer.edit', ['id' => $customer->id]) }}" data-bs-toggle="tooltip" title="{{ get_phrase('Edit') }}" class="btn btn-primary-light btn-icon"><i class="fi-rr-pencil"></i></a>
                                            <a href="javascript:;" onclick="confirmModal('{{ route('admin.customer.delete', ['id' => $customer->id]) }}')" data-bs-toggle="tooltip" title="{{ get_phrase('Delete') }}" class="btn btn-danger-light btn-icon"><i class="fi-rr-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                ...
                <div class="admin-tInfo-pagi d-flex justify-content-md-between justify-content-center align-items-center gr-15 flex-wrap">
                    <p class="admin-tInfo">
                        {{ get_phrase('Showing') . ' ' . count($customers) . ' ' . get_phrase('of') . ' ' . $customers->total() . ' ' . get_phrase('data') }}
                    </p>
                </div>
