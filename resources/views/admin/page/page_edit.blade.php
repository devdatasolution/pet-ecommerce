...
@push('title', get_phrase('Page edit'))
@push('meta')
@endpush
@push('css')
@endpush
                ...
                <div class="ol-card-body">
                    <form action="{{ route('admin.page.update', ['id' => $page->id]) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" value="{{ $page->title }}" name="title" class="form-control ol-form-control" id="title" placeholder="{{ get_phrase('Enter page title') }}" aria-label="{{ get_phrase('Enter page title') }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="content" class="form-label ol-form-label">{{ get_phrase('Page content') }}</label>
                            <textarea name="content" rows="4" class="form-control ol-form-control text_editor" id="content" placeholder="{{ get_phrase('Write your page content') }}">{{ $page->content }}</textarea>
                        </div>

                        <div class="mb-3">
                            <label for="status" class="form-label ol-form-label">{{ get_phrase('Status') }}</label>
                            <select class="ol-select2" name="status" id="status">
                                <option value="0" @if ($page->status == 0) selected @endif>{{ get_phrase('Inactive') }}</option>
                                <option value="1" @if ($page->status == 1) selected @endif>{{ get_phrase('Active') }}</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" value="{{$page->slug}}" class="form-control" name="slug" id="slug" aria-describedby="page_link2 page_link3">
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="ol-select2" name="button_placement" id="button_placement">
                                <option value="footer" @if ($page->button_placement == 0) selected @endif>{{ get_phrase('Footer') }}</option>
                                <option value="header" @if ($page->button_placement == 1) selected @endif>{{ get_phrase('Header') }}</option>
                            </select>
                        </div>
