...
@push('title', get_phrase('Store Edit'))
@push('meta')
@endpush
@push('css')
@endpush
            ...
            <div class="ol-card p-4">
                <div class="ol-card-body">
                    <form action="{{ route('admin.store.update', ['id' => $store->id]) }}" method="post" enctype="multipart/form-data">
                        ...
                        {{-- <div class="mb-3">
                            <label for="store_id" class="form-label ol-form-label">{{ get_phrase('Store owner') }}</label>
                            <select class="ol-select2" name="user_id" id="store_id">
                                <option value="">{{ get_phrase('Select store owner') }}</option>
                                @foreach ($customers as $customer)
                                    <option value="{{ $customer->id }}" @if($customer->id == $store->user_id) selected @endif>{{ $customer->name }} ({{ $customer->email }})</option>
                                @endforeach
                            </select>
                        </div> --}}
                          @auth
                            @if(auth()->user()->role_id == 1 && auth()->id() != $store->user_id)
                                {{-- Admin, but editing someone else's store --}}
                                <div class="mb-3">
                                    <label for="store_id" class="form-label ol-form-label">
                                        {{ get_phrase('Store owner') }}
                                    </label>
                                    ...
                                    <select class="ol-select2" name="user_id" id="store_id">
                                        ...
                                        @foreach ($customers as $customer)
                                            <option value="{{ $customer->id }}"
                                                @if($customer->id == $store->user_id) selected @endif>
                            ...
                            @else
                                {{-- Admin editing own store OR non-admin --}}
