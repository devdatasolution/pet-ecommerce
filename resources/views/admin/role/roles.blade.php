...
@push('title', get_phrase('User Roles'))
@push('meta')
@endpush
@push('css')
@endpush
    ...
    <div class="ol-card">
        <div class="ol-card-body mt-1rem pt-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                ...
                <a href="{{ route('admin.role.add') }}" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px">
                    ...
                    <table class="table print-table">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">{{ get_phrase('Permissions') }}</th>
                        ...
                        <tbody>
                            @foreach ($roles as $key => $role)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-light text-dark">Permissions</span>
                                    </td>
                                    <td>
                                        <div class="print-d-none d-flex flex-wrap align-items-center gap-2 row-gap-2">
                                            <a href="{{ route('admin.role.edit', ['id' => $role->id]) }}" data-bs-toggle="tooltip" title="{{ get_phrase('Edit') }}" class="btn btn-primary-light btn-icon"><i class="fi-rr-pencil"></i></a>
                                            <a href="#" onclick="confirmModal('{{ route('admin.role.delete', ['id' => $role->id]) }}')" data-bs-toggle="tooltip" title="{{ get_phrase('Delete') }}" class="btn btn-danger-light btn-icon"><i class="fi-rr-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
