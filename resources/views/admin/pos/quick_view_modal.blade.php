...
<div class="quick_cart">
    <div class="d-flex gap-3 flex-column flex-lg-row">
        <div class="quick-view-banner-wrap">
            <img class="image-zoom-active" src="{{ get_image($product->thumbnail) }}" alt="thumbnail">
        </div>
        <div class="w-100">
            ...
            <div class="d-flex align-items-end justify-content-between gap-10px flex-wrap mb-4">
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    <div class="d-flex align-items-center gap-12px">
                        ...
                        @if ($product->is_discounted)
                            ...
                            @endphp
                            @if ($discount->discount_type == 'percentage')
                                <p class="sky-blue-badge-sm px-2">-{{ $discount->discount_value }}%</p>
                            @endif
                        @endif
                    </div>
                ...
                <div class="d-flex align-items-center gap-6px">
                    <a href="#" class="circle-iconbox-24px">
                    ...
                    <a href="#" class="circle-iconbox-24px">
                        <span class="d-flex align-items-center justify-content-center w-100 h-100 rounded-circle" data-bs-toggle="tooltip" data-bs-title="Share">
                            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="12" viewBox="0 0 13 12" fill="none">
            ...
            <div class="mb-4">
                ...
                <div class="progress fsh-progress-sm mb-2 max-w-254px" role="progressbar" aria-label="Basic example" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100">
                    <div class="progress-bar" ></div>
                </div>
                
            </div>
            <div class="cartForm">
                @if(!empty($product->product_attribute_types))
                <div class="mb-3">
                    ...
                    <div class="mb-4">
                        <div class="d-flex gap-20px flex-wrap">
                            @if ($attribute_type->input_type == 'color')
                                <div>
                                    <h6 class="al-title-12px fw-medium mb-3"><span class="fsh-text-gray">{{ $attribute_type->name }} :</span> Light Pink </h6>
                ...
                @endforeach
                <button type="submit" class="btn fsh-sm-btn-dark w-100 mb-2" id="itemsSetProductForm" data-category-id="{{ $product->category_id }}" data-id="{{ $product->id }}" data-title="{{ $product->title }}" data-price="{{ $product->price }}">
...
$(document).ready(function () {
    // Reset product form whenever modal is opened
    $('#quickViewModal').off('shown.bs.modal').on('shown.bs.modal', function () {
        let $form = $(this).find('#productForm');

        // Reset form fields
        $form[0].reset();

        // Reset quantity to 1
        $form.find('.quantity-of-product').val(1);

        // Reset attribute selections (first option selected)
        $form.find('input[type=radio]').each(function (i, el) {
            let $el = $(el);
            if (i === 0 || $el.is('[checked]')) {
    ...
    $(document).off('click', '.quantity-btn').on('click', '.quantity-btn', function () {
        var $button = $(this);
        var $input = $button.closest('.quantity-input-wrap').find(".quantity-of-product");
        var oldValue = parseFloat($input.val()) || 0;

        if ($button.hasClass("inc")) {
            var newVal = oldValue + 1;
        } else {
            newVal = oldValue > 1 ? oldValue - 1 : 1; // Minimum 1
        }
