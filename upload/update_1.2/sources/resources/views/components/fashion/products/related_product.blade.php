              ...
              <div class="d-block product-grid-md swiper-slide">
                        <div>
                            <div class="product-grid-banner-md mb-12px">
                                ...
                                <a href="{{ route('product', $related_product->slug) }}" class="btn fsh-btn-dark product-cart-btn-md">{{get_phrase('Shop Now')}}</a>
                                <a href="javascript:void(0)" class="product-wishlist-btn {{ wishlist_class($related_product->id) }}" 
                                ...
                                <a href="javascript:;" class="product-quickview-btn" onclick="load_view('{{ route('view', ['path' => 'frontend.products.quick_view', 'product_id' => $related_product->id]) }}', '#quickViewModal .modal-body')" data-bs-toggle="modal" data-bs-target="#quickViewModal">
                            ...
                            <div>
                                <a href="{{ route('product', $related_product->slug) }}" class="al-title-16px mb-12px product-title-link">{{ \Illuminate\Support\Str::limit($related_product->title, 70, '...') }}</a>
                                    <div class="d-flex justify-content-between">
                                        <div class="d-flex align-items-start gap-1 mb-12px">
                                            ...
                                            <h6 class="al-title-12px fw-medium">{{ number_format($related_product->average_rating, 1) }}</h6>
                                        </div>
                                    <div class="d-flex align-items-start gap-2">
                                        @if ($related_product->is_discounted()->exists())
                                                @if ($related_product->is_discounted->discount_type == 'flat')
                                                    <div class="d-flex gap-2">
                                                        <h6 class="al-title-16px">  {{ currency($related_product->price - $related_product->is_discounted->discount_value) }} </h6>
                                                ...
                                                @else
                                                    ...
                                                    @endphp
                                                    <div class="d-flex gap-2">
                                                        <h6 class="al-title-16px"> {{ currency($related_product->price - $discount_amount) }}  </h6>
                                                            <h6 class="al-title-16px fw-medium fsh-text-gray"><del>{{ currency($related_product->price) }}</del></h6>
                                                    </div>
                                                    
                                                @endif
                                            @else
                                                    <h6 class="al-title-16px">{{ currency($related_product->price) }}</h6>
                                            @endif
                                    </div>
                                </div>
                            </div>
