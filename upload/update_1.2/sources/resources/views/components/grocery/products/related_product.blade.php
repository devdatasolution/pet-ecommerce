...
<div class="swiper products-slider">
    <div class="swiper-wrapper">
            ...
            @foreach ($product->related_products()->where('status', 1)->take(30)->get() as $related_product)
                <div class="product-md-card swiper-slide">
                           ...
                           @if ($related_product->is_discounted()->exists())
                                @php
                                    ...
                                    if ($discount->discount_type === 'percentage') {
                                        ...
                                        $discount_text = currency($discount->discount_value) . ' OFF';
                                    }
                                @endphp
                            ...
                            <div class="view-bookmark-md-wrap">
                                <a href="javascript:void(0)" class="dark-light-iconbtn"  onclick="load_view('{{ route('view', ['path' => 'frontend.products.quick_view', 'product_id' => $related_product->id]) }}', '#quickViewModal .modal-body')" data-bs-toggle="modal" data-bs-target="#quickViewModal">
                                ...
                                <a href="javascript:void(0)" class="dark-light-iconbtn {{ wishlist_class($related_product->id) }}" 
                        ...
                        <div class="product-md-details">
                            <a href="{{ route('product', $related_product->slug) }}" class="al-title-16px text-link mb-12px">{{ $related_product->title }}</a>
                            <div class="d-flex align-items-center gap-1 flex-wrap mb-1">
                                ...
                                <p class="al-title2-12px">{{ $related_product->average_rating ?? 0 }} <span class="ec-text-gray">({{ $related_product->reviews->count() }})</span></p>
                            </div>
                                    ...
                                    @if ($related_product->is_discounted()->exists())
                                        @if ($related_product->is_discounted->discount_type == 'flat')
                                            <div class="d-flex gap-2">
                                                <h6 class="al-title-18px">  {{ currency($related_product->price - $related_product->is_discounted->discount_value) }} </h6>
                                                <h6 class="al-title-16px fw-medium ec-text-gray"><del>{{ currency($related_product->price) }}</del></h6>
                                            </div>
                                        @else
                                            @php
                                            ...
                                            @endphp
                                            <div class="d-flex gap-2">
                                                <h6 class="al-title-18px"> {{ currency($related_product->price - $discount_amount) }}  </h6>
                                                    <h6 class="al-title-16px fw-medium ec-text-gray"><del>{{ currency($related_product->price) }}</del></h6>
                                            </div>
                                            
                                        @endif
                                    @else
                                            <h6 class="al-title-18px">{{ currency($related_product->price) }}</h6>
                                    @endif
                                </div>
                                <a href="{{ route('product', $related_product->slug) }}" class="btn ec-sm2-btn-dark text-capitalize">
