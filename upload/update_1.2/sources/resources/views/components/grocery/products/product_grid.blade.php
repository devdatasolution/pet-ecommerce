@php
    if($layout == 'lg')
    {
        $title_class = 'al-title-20px  text-link mb-12px';
        ...
        $price_del_class = 'al-title-16px fw-medium ec-text-gray';
        $btn_class = 'btn ec-sm2-btn-dark text-capitalize py-12px px-4';
    } 
    elseif($layout == 'md')
    {
        $title_class = 'al-title-16px  text-link mb-12px';
        $star_class_fill = 'star-yellow-15.svg';
        $star_class = 'star-yellow-stroke-15.svg';
        $rating_class = 'al-title2-12px';
        $price_class = 'al-title-18px';
        $price_del_class = 'al-title-16px fw-medium ec-text-gray';
        $btn_class = 'btn ec-sm2-btn-dark text-capitalize';
    } 
    else
    {
        $title_class = 'al-title2-14px  text-link mb-2';
        $star_class_fill = 'star-yellow-15.svg';
        $star_class = 'star-yellow-stroke-15.svg';
        $rating_class = 'al-title2-12px';
        $price_class = 'al-title2-14px';
        $price_del_class = 'al-title2-12px fw-medium ec-text-gray';
        $btn_class = 'btn ec-sm3-btn-dark text-capitalize';
    }
@endphp
<div class="product-{{ $layout }}-card">
    <div class="product-{{ $layout }}-banner">
        @if ($product->is_discounted()->exists())
            ...
            @endphp
            <p class="product-discount-badge1 capitalize">{{ $discount_text }}</p>
            @endif
        @php
        ...
        <div class="view-bookmark-{{ $layout == 'lg' ? 'md' : $layout }}-wrap">
            <a href="javascript:void(0)" onclick="load_view('{{ route('view', ['path' => 'frontend.products.quick_view', 'product_id' => $product->id]) }}', '#quickViewModal .modal-body')"  class="dark-light-iconbtn" data-bs-toggle="modal" data-bs-target="#quickViewModal">
                <span class="d-flex align-items-center justify-content-center w-100 h-100 rounded-circle" data-bs-toggle="tooltip" data-bs-title="Quick View" aria-describedby="tooltip276572">
    ...
    <div class="product-{{ $layout }}-details">
        ...
        <div class="d-flex align-items-center gap-1 flex-wrap mb-6px">
            ...
            <p class="{{ $rating_class }}">{{ $product->average_rating ?? 0 }} <span class="ec-text-gray">({{ $product->reviews->count() }})</span></p>
        </div>
        <div class="d-flex align-items-end gap-3 justify-content-between flex-wrap">
            ...
            <a href="{{ route('product', $product->slug) }}" class="{{ $btn_class }}">
