...
@push('title', get_phrase('Vendor Payout'))
@push('meta')
@endpush
@push('css')
@endpush
                ...
                <div class="col-md-12">
                    ...
                    <div class="tab-content eNav-Tabs-content" id="myTabContent">
                        <div class="tab-pane fade @if(!isset($_GET['eDateRange'])) show active @endif" id="cHome" role="tabpanel" aria-labelledby="cHome-tab">
                            <div class="row print-d-none mt-4 row-gap-3">
                                <div class="col-md-6 pt-2 pt-md-0">
                                    @if (count($vendor_payout_incomplete) > 0)
                                ...
                                <div class="col-md-6">
                                    <form class="form-inline" action="{{ route('admin.vendor.payout.filter') }}" method="get">
                                        <div class="row mb-4">
                                            <div class="col-md-9">
                            ...
                            @if (count($vendor_payout_incomplete) > 0)
                                ...
                                <div class="table-responsive purchase_list mt-4" id="purchase_list">
                                    <table class="table eTable eTable-2 print-table">
                                        ...
                                        <tbody>
                                            @foreach ($vendor_payout_incomplete as $key => $vendor_payout_incompleted)
                                                <tr>
                                                    <th scope="row">
                                                    ...
                                                    <td>
                                                        <div class="dAdmin_profile d-flex align-items-center min-w-200px">
                                                            <div class="dAdmin_profile_img">
                                                                <img class="img-fluid rounded-circle image-45" width="45" height="45" src="{{ get_image($vendor_payout_incompleted->user_id) }}" />
                                                            </div>
                                                            <div class="ms-1">
                                                                <h4 class="title fs-14px">{{ get_user_info($vendor_payout_incompleted->user_id)->name }}</h4>
                                                                <p class="sub-title2 text-12px">{{ get_user_info($vendor_payout_incompleted->user_id)->email }}</p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div class="dAdmin_info_name min-w-250px mr-50">
                                                            <p>{{ currency($vendor_payout_incompleted->amount) }}
                                                            </p>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="dAdmin_info_name min-w-250px">
                                                            <p>{{ date('D, d M Y', strtotime($vendor_payout_incompleted->created_at)) }}
                                                            </p>
                                                        </div>
                                                    </td>

                                                    <td class="print-d-none">
                                                        <form action="{{ route('admin.vendor.payment') }}" method="post">
                                                            @csrf
                                                            <input type="hidden" name="user_id" value="{{ $vendor_payout_incompleted->user_id }}">
                                                            <input type="hidden" name="amount" value="{{ $vendor_payout_incompleted->amount }}">
                                                            <input type="hidden" name="payout_id" value="{{ $vendor_payout_incompleted->id }}">
                            ...
                            @if (count($vendor_payout_incomplete) > 0)
                                <div class="admin-tInfo-pagi d-flex justify-content-between justify-content-center align-items-center flex-wrap gr-15">
                                    <p class="admin-tInfo">
                                        {{ get_phrase('Showing') . ' ' . count($vendor_payout_incomplete) . ' ' . get_phrase('of') . ' ' . $vendor_payout_incomplete->total() . ' ' . get_phrase('data') }}
                                    </p>
                                    {{ $vendor_payout_incomplete->links() }}
                                </div>
                            @endif
                        </div>
                        <div class="tab-pane fade @if(isset($_GET['eDateRange'])) show active @endif" id="cProfile" role="tabpanel" aria-labelledby="cProfile-tab">
                            ...
                            <div class="row print-d-none mt-4 row-gap-3">
                                <div class="col-md-6 pt-2 pt-md-0">
                                    @if (count($vendor_payout_complete) > 0)
                                        <div class="custom-dropdown">
                                            <button class="dropdown-header btn ol-btn-light">
                                                {{ get_phrase('Export') }}
                                                <i class="fi-rr-file-export ms-2"></i>
                            ...
                            @if (count($vendor_payout_complete) > 0)
                                ...
                                <div class="table-responsive purchase_list" id="purchase_list">
                                    <table class="table eTable eTable-2 print-table">
                                        ...
                                        <tbody>
                                            @foreach ($vendor_payout_complete as $key => $vendor_payouts)
                                                <tr>
                                                    <th scope="row">
                                                        <p class="row-number">{{ ++$key }}</p>
                                                    </th>
                                                    <td>
                                                        <div class="dAdmin_profile d-flex align-items-center min-w-200px">
                                                            <div class="dAdmin_profile_img">
                                                                <img class="img-fluid rounded-circle image-45" width="45" height="45" src="{{ get_image($vendor_payouts->user_id) }}" />
                                                            </div>
                                                            <div class="ms-1">
                                                                <h4 class="title fs-14px">{{ get_user_info($vendor_payouts->user_id)->name }}</h4>
                                                                <p class="sub-title2 text-12px">{{ get_user_info($vendor_payouts->user_id)->email }}</p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div class="dAdmin_info_name min-w-250px">
                                                            <p>{{ currency($vendor_payouts->amount) }}</p>

                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="dAdmin_info_name min-w-250px">
                                                            @if ($vendor_payouts->status != 0)
                                                                <p> {{ get_phrase('Paid') }} </p>
                                                            @else
                                                    ...
                                                    <td>
                                                        <div class="dAdmin_info_name min-w-250px">
                                                            <p>{{ date('D, d M Y', strtotime($vendor_payouts->updated_at)) }}
                                                            </p>
                            ...
                            @if (count($vendor_payout_complete) > 0)
                                <div class="admin-tInfo-pagi d-flex justify-content-between justify-content-center align-items-center flex-wrap gr-15">
                                    <p class="admin-tInfo">
                                        {{ get_phrase('Showing') . ' ' . count($vendor_payout_complete) . ' ' . get_phrase('of') . ' ' . $vendor_payout_complete->total() . ' ' . get_phrase('data') }}
                                    </p>
                                    {{ $vendor_payout_complete->links() }}
                                </div>
                            @endif
                        </div>
                    </div>
