...
class SSLCommerz extends Model
{
    ...
    public static function payment_create($identifier)
    {
        ...
        $payment_info = array($user->id, $payment_details, microtime(true));
        $payment_info = json_encode($payment_info);
        $payment_info = base64_encode($payment_info);
        $payment_info = str_replace("=", "", $payment_info);
        DB::table('users')->where('id', $user->id)->update(['temp' => $payment_info]);
        ...
        $post_data['success_url']  = $payment_details['success_url'] . '/' . $payment_gateway->identifier . '?i=' . $payment_info;
