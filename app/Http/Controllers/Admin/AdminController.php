...
{
    // ===== Counts =====
    $page_data['totalProduct']  = Product::where('status', 1)->count();
    $page_data['totalCustomer'] = User::where('user_type', 'customer')->count();
    $page_data['totalOrder']    = Order::count();
    $page_data['totalStore']    = Store::count();
    $page_data['stores']        = Store::get();
    ...
    // Load product with discount
    $productDetails = Product::whereIn('id', $productIds)
        ->with('discount')
        ->get()
        ->keyBy('id');

    // Prepare data for Blade (NO Blade change needed)
    $topProducts = $topProducts->map(function ($item) use ($productDetails) {
       ...
       $discount_type  = null;
        ...
        // }
        if ($product && $product->discount && $product->discount->discount_value > 0) {
            $discount_type  = $product->discount->discount_type;
            $discount_value = $product->discount->discount_value;
        }
        // Calculate final price only for total sales
        $final_price = $price;
        ...
        return (object) [
            ...
            // Blade depends on these
            'price'          => $price,
            'discount_type'  => $discount_type,
            'discount_value' => $discount_value,

            'total_quantity' => $item->total_quantity,
            'total_sales'    => round($final_price * $item->total_quantity, 2),
        ];
    });
    ...
    public function category_delete($id)
{
    ...
    public function vendor_settings(){
         $page_data['page_title'] = get_phrase('Revenue  Settings');
        $page_data['vendor_revenue'] = Setting::where('type', 'vendor_revenue')->first();
        return view('admin.vendor.vendor_settings', $page_data);
    }
   
    public function vendor_settings_update(Request $request){
        $key_found = Setting::where('type', 'vendor_revenue')->exists();
        if ($key_found) {
            $data['description'] = $request->vendor_revenue;
            Setting::where('type', 'vendor_revenue')->update($data);
        } else {
            $data['type']        = 'vendor_revenue';
            $data['description'] = $request->vendor_revenue;

            Setting::insert($data);
        }
        Session::flash('success', get_phrase('Vendor setting updated'));
        return redirect()->back();
    }
     ...
     public function vendor_payout(Request $request)
    {
        ...
        $page_data['start_date']                 = $start_date;
        $page_data['end_date']                   = $end_date;
        $page_data['vendor_payout_complete'] = Payout::where('status', 1)->where('created_at', '>=', date('Y-m-d H:i:s', $start_date))
            ->where('created_at', '<=', date('Y-m-d H:i:s', $end_date))->paginate(10);
        $page_data['vendor_payout_incomplete'] = Payout::where('status', 0)->where('created_at', '>=', date('Y-m-d H:i:s', $start_date))
            ->where('created_at', '<=', date('Y-m-d H:i:s', $end_date))->paginate(10);
        $page_data['page_title'] = get_phrase('Vendor Payout');
        return view('admin.vendor.payout', $page_data);
    }

 public function vendor_payout_filter(Request $request)
    {
        ...
        $page_data['vendor_payout_complete'] = Payout::where('status', 1)->where('created_at', '>=', date('Y-m-d H:i:s', $start_date))
            ->where('created_at', '<=', date('Y-m-d H:i:s', $end_date))->paginate(10);
        $page_data['vendor_payout_incomplete'] = Payout::where('status', 0)->paginate(10);
     ...
     public function vendor_payment(Request $request)
    {
        ...
        $payment_details = [
            'items'          => [
                [
                    'id'                  => $id,
                    'title'               => get_phrase('Pay for vendor payout'),
                    'name'                => get_phrase('Vendor payout'),
                    'subtitle'            => get_phrase(''),
            ...
            'success_method' => [
                ...
                'function_name' => 'vendor_payment',
            ],
            'tax'            => 0,
            'coupon'         => null,
            'payable_amount' => $payable_amount,
            'cancel_url'     => route('admin.vendor.payout'),
