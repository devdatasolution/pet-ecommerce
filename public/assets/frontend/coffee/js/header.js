$(document).ready(function(){
    ...
    var smallHeaderSticky = function () {
        ...
        function handleScroll() {
            ...
            let navbarHeight = $(".cts-header-section").outerHeight();

            if (Math.abs(lastScrollTop - st) <= delta) return;

            if (st > lastScrollTop && st > navbarHeight) {
                $(".cts-header-section").css("top", `-${navbarHeight + 3}px`);
            } else {
                if (st + $(window).height() < $(document).height()) {
                    $(".cts-header-section").css("top", "0px");
                }
            }
            lastScrollTop = st;
        }
    ...
    var smallHeaderChangeBg = function () {
        function handleBgChange() {
            $(".cts-header-section").toggleClass("header-bg", $(window).scrollTop() > 100);
        }
    ...
    var handleResize = function () {
        if ($(window).width() < 992) {
            ...
            $(".cts-header-section").removeAttr("style").removeClass("header-bg"); // Reset styles for larger screens
        }
    };
