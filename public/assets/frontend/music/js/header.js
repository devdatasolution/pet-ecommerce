$(document).ready(function(){
    if ($('.mi-nice-select').length > 0) {
        $('.mi-nice-select').niceSelect('destroy');
        $('.mi-nice-select').niceSelect(); 
    }
    ...
    var smallHeaderSticky = function () {
        ...
        function handleScroll() {
            ...
            let navbarHeight = $(".mi-header-section").outerHeight();

            if (Math.abs(lastScrollTop - st) <= delta) return;

            if (st > lastScrollTop && st > navbarHeight) {
                $(".mi-header-section").css("top", `-${navbarHeight + 3}px`);
            } else {
                if (st + $(window).height() < $(document).height()) {
                    $(".mi-header-section").css("top", "0px");
                }
            }
            lastScrollTop = st;
        }
    ...
    var smallHeaderChangeBg = function () {
        function handleBgChange() {
            $(".mi-header-section").toggleClass("header-bg", $(window).scrollTop() > 100);
        }
    ...
    var handleResize = function () {
        if ($(window).width() < 992) {
            ...
            $(".mi-header-section").removeAttr("style").removeClass("header-bg"); // Reset styles for larger screens
        }
    };
