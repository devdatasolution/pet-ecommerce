$(document).ready(function(){
    ...
    var smallHeaderSticky = function () {
        ...
        function handleScroll() {
            ...
            let navbarHeight = $(".pf-header-section").outerHeight();

            if (Math.abs(lastScrollTop - st) <= delta) return;

            if (st > lastScrollTop && st > navbarHeight) {
                $(".pf-header-section").css("top", `-${navbarHeight + 3}px`);
            } else {
                if (st + $(window).height() < $(document).height()) {
                    $(".pf-header-section").css("top", "0px");
                }
            }
            lastScrollTop = st;
        }
    ...
    var smallHeaderChangeBg = function () {
        function handleBgChange() {
            $(".pf-header-section").toggleClass("header-bg", $(window).scrollTop() > 100);
        }
    ...
    var handleResize = function () {
        if ($(window).width() < 992) {
            ...
            $(".pf-header-section").removeAttr("style").removeClass("header-bg"); // Reset styles for larger screens
        }
    };
