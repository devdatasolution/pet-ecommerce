$(document).ready(function(){
    ...
    var smallHeaderSticky = function () {
        ...
        function handleScroll() {
            ...
            let navbarHeight = $(".mc-header-section").outerHeight();

            if (Math.abs(lastScrollTop - st) <= delta) return;

            if (st > lastScrollTop && st > navbarHeight) {
                $(".mc-header-section").css("top", `-${navbarHeight + 3}px`);
            } else {
                if (st + $(window).height() < $(document).height()) {
                    $(".mc-header-section").css("top", "0px");
                }
            }
            lastScrollTop = st;
        }
    ...
    var smallHeaderChangeBg = function () {
        function handleBgChange() {
            $(".mc-header-section").toggleClass("header-bg", $(window).scrollTop() > 100);
        }
    ...
    var handleResize = function () {
        if ($(window).width() < 992) {
            ...
            $(".mc-header-section").removeAttr("style").removeClass("header-bg"); // Reset styles for larger screens
        }
    };
