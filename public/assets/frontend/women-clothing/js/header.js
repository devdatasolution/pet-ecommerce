$(document).ready(function(){
    ...
    $('.mobile-menuitem-a-have-sub, .mobile-dropitem-a-have-sub').on('click', function (e) {
        e.preventDefault();
        var $this = $(this);
        var $parentLi = $this.parent('li');
        var $submenu = $parentLi.children('ul');
        $submenu.slideToggle();
        $parentLi.toggleClass('active-mobile-sub-submenu');
        $parentLi
            .siblings()
            .removeClass('active-mobile-sub-submenu')
            .children('ul')
            .slideUp();
    });
...
accordion2();
    ...
    var smallHeaderSticky = function () {
        ...
        function handleScroll() {
            ...
            let navbarHeight = $(".wc-header-section").outerHeight();

            if (Math.abs(lastScrollTop - st) <= delta) return;

            if (st > lastScrollTop && st > navbarHeight) {
                $(".wc-header-section").css("top", `-${navbarHeight + 3}px`);
            } else {
                if (st + $(window).height() < $(document).height()) {
                    $(".wc-header-section").css("top", "0px");
                }
            }
            lastScrollTop = st;
        }
    ...
    var smallHeaderChangeBg = function () {
        function handleBgChange() {
            $(".wc-header-section").toggleClass("header-bg", $(window).scrollTop() > 100);
        }
    ...
    var handleResize = function () {
        if ($(window).width() < 992) {
            ...
            $(".wc-header-section").removeAttr("style").removeClass("header-bg"); // Reset styles for larger screens
        }
    };
